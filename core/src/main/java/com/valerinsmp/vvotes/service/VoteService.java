...
public final class VoteService {
    ...
    public VoteService(
        ...
        this.monthlyDrawService = new MonthlyDrawService(plugin, configService, messageService, soundService, repo, rewardExecutor, platformAdapter);
    }
        ...
        Bukkit.getScheduler().runTaskAsynchronously(plugin, () -> {
            ...
            Bukkit.getScheduler().runTask(plugin, () -> {
                ...
                if (online != null && online.isOnline()) {
                    messageService.send(online, "vote-pending-delivered", Map.of("amount", Integer.toString(rows.size())));
                }
            });
        });
    }
                ...
                if (!globalDailyReached.isEmpty() || !recurringReached.isEmpty()) {
                    Bukkit.getScheduler().runTask(plugin, () -> {
                        for (Integer threshold : globalDailyReached) {
                            ...
                            for (String line : messageService.messages("global-goal-completed-broadcast", gp)) {
                                platformAdapter.broadcast(line);
                            }
                        ...
                        for (Integer threshold : recurringReached) {
                            ...
                            for (String line : messageService.messages("global-recurring-goal-completed-broadcast", rp)) {
                                platformAdapter.broadcast(line);
                            }
    ...
    private void deliverNotificationsAndRewards(
        ...
        Bukkit.getScheduler().runTask(plugin, () -> {
            if (config.broadcastOnVote()) {
                for (String line : messageService.messages("vote-broadcast", placeholders)) {
                    for (Player online : Bukkit.getOnlinePlayers()) {
                        if (isVoteAnnouncementMuted(online.getUniqueId(), online.getName())) {
                            continue;
                        }
                        platformAdapter.send(online, line);
                    }
                }
                for (Player online : Bukkit.getOnlinePlayers()) {
                    if (isVoteAnnouncementMuted(online.getUniqueId(), online.getName())) {
            ...
            if (player != null) {
                ...
                for (Integer threshold : monthlyGoalsReached) {
                    ...
                    for (String line : messageService.messages("player-monthly-goal-completed", mp)) {
                        platformAdapter.send(player, line);
                    }
                }
                ...
                if (doubleSiteBonusCompleted) {
                    ...
                    if (bonusMessage != null && !bonusMessage.isBlank()) {
                        ...
                        platformAdapter.send(player, messageService.applyPlaceholders(withPrefix, placeholders));
                    }
                }
